...
function ProjectLogo({ project, isActive, size }) {
  ...
  const h = size === 'large' ? (isVideo ? '750px' : '400px') : isVideo ? '340px' : project.id === 'djamao' ? '110px' : '64px'
  ...
  if (project.logoType === 'video') {
    const blockShift = size !== 'large' ? 'translate(10px, -97px)' : 'translate(35px, -130px) scale(1.5)'
    return (
